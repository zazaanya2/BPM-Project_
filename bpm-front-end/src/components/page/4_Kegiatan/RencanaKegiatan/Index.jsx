...
const Index = () => {
    const [events, setEvents] = useState([
        ...
        { id: 1, title: 'Pelaksanaan Audit Eksternal', description: 'Deskripsi audit eksternal', start: new Date(2024, 9, 16), end: new Date(2024, 9, 16), location: 'Politeknik Astra' }
    ]);
    ...
    const handleAddEvent = () => {
        ...
        setFormData({ name: '', description: '', startDate: '', startTime: '', endDate: '', endTime: '', location: '' });
    ...
    return (
        <div style={{ display: 'flex', marginTop: '150px' }}>
            <div style={{ flex: 2 }}>
                <Button label="Tambah Rencana" onClick={() => addModalRef.current.open()} />
                <Calendar
                    localizer={localizer}
                    events={events}
                    startAccessor="start"
            ...
            <div style={{ flex: 1, padding: '20px', borderLeft: '1px solid #ddd' }}>
                {selectedEvent ? (
                    <div>
                        ...
                        <p><strong>Tanggal:</strong> {moment(selectedEvent.start).format('dddd, DD MMMM YYYY')}</p>
                        <p><strong>Deskripsi:</strong> {selectedEvent.description}</p>
                        <p><strong>Tempat:</strong> {selectedEvent.location}</p>
                        <Button label="Edit" onClick={() => updateModalRef.current.open()} />
                    </div>
                ) : (
